...
(* 1st version : very few changes from Array *)

let cutoff = 5
let unsafe_stable_sort_sub_1 cmp a init_ofs init_len =
...
(* 2nd version : uses unsafe_get for better execution time and raising errors
  if the array's length is modified *)

let unsafe_stable_sort_sub_2 cmp a init_ofs init_len =
...
(* 3rd version : much shorter and supposedly faster than the previous ones *)

let unsafe_stable_sort_sub_3 cmp a init_ofs init_len =
  let Pack {arr = arr1; length = len1; dummy = dum1} = a in
  check_valid_length len1 arr1;
...
(* Public functions, common to the three versions *)

let stable_sort_sub cmp a ofs len version =
  if ofs < 0 || len < 0 || ofs > length a - len
  then invalid_arg Error.invalid_state_description
  else match version with
    | 1 -> unsafe_stable_sort_sub_1 cmp a ofs len
    | 2 -> unsafe_stable_sort_sub_2 cmp a ofs len
    | 3 -> unsafe_stable_sort_sub_3 cmp a ofs len
    | _ -> invalid_arg "Argument version should be 1, 2 or 3"

let stable_sort cmp a version =
  let Pack {arr; length = len; _} = a in begin
  check_valid_length len arr;
  stable_sort_sub cmp a 0 (length a) version;
